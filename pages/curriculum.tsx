...
const ScrollArrow: React.FC<{ scrolledRight: boolean }> = ({
  ...
  return (
    <img
      src={`/assets/curriculum/icons/scroll-${scrolledRight ? 'left' : 'right'
        }.svg`}
      className={`${styles['arrow']} ${scrolledRight && styles['right']
...
export const Curriculum: React.FC<Props> = ({ lessons, alerts }) => {
  ...
  const lessonsToRender: React.ReactElement[] = lessons.map((lesson, idx) => {
    ...
    return (<>
      {/* <LessonCertificate passedAt={passedAt} title={title}/> */}
