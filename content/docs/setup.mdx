...
Create an account on [pathacks.com/signup](/signup) if you don't already have one.
...
2. Create a new branch from master (pro tip: `p1` for problem 1):
   `git checkout -b p1`.
3. Go to [pathacks.com](/), click on Curriculum, select lesson 0, and read
   challenge 1.
...
7. Submit your solution: `c0d3 submit`
   ...
   - Go back to your student page (pathacks.com → Curriculum → lesson 0), refresh
...
### Reusing your branches


1. Go to an existing branch that you plan to use for problem 1 (e.g. `p1`)
    - `git checkout p1`
        - (Notice how we did not add a `-b` flag because we are not creating a new branch)
2. Update this branch with all the code in master: `git merge master` 
3. Write the solution by opening and editing `js1/1.js` 
4. Test your file by running `npm run test` (make sure tests are passing 100%)
5. Add and commit changed files
    - `git add js1/1.js`
    - `git commit -m "solves problem 1"` → saves the changes
        - If you forget to put `-m`, you will be put into vim. To get out, type `:wq` to save and exit.
    - `c0d3 submit` to upload your work to [pathacks.com](https://pathacks.com) for code review
6. Start from step 1 and repeat for the next challenge
...
[JS 0 - Foundations](https://www.pathacks.com/curriculum/js0/primitive_data_and_operators)
